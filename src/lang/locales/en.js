const traitInputAttr = { placeholder: 'eg. Text here' };

export default {
  assetManager: {
    addButton: 'إضافة صورة',
    inputPlh: 'http://path/to/the/image.jpg',
    modalTitle: 'اختر صورة',
    uploadTitle: 'قم بإسقاط الملفات هنا أو انقر للتحميل',
  },
  // Here just as a reference, GrapesJS core doesn't contain any block,
  // so this should be omitted from other local files
  blockManager: {
    labels: {
      // 'block-id': 'Block Label',
      'link-block': 'رابط بلوك',
      'quote': 'يقتبس',
      'text-basic': 'قسم النص',
      'column1': '1 عمود',
      'column2': '2 العمود',
      'column3': '3 عمود',
      'column3-7': '4 عمود',
      'text': 'نص',
      'link': 'وصلة',
      'image': 'صورة',
      'video': 'فيديو',
      'map': 'خريطة'
    },
    categories: {
      // 'category-id': 'Category Label',
    },
  },
  domComponents: {
    names: {
      '': 'علبة',
      wrapper: 'الجسم',
      text: 'نص',
      comment: 'تعليق',
      Image: 'صورة',
      video: 'فيديو',
      label: 'مُلصَق',
      link: 'وصلة',
      map: 'خريطة',
      tfoot: 'قدم الطاولة',
      tbody: 'جسم الجدول',
      thead: 'رأس الجدول',
      table: 'الطاولة',
      row: 'صف الجدول',
      cell: 'خلية الجدول',
    },
  },
  deviceManager: {
    device: 'جهاز',
    devices: {
      desktop: 'سطح المكتب',
      tablet: 'لوح',
      mobileLandscape: 'المناظر الطبيعية للجوال',
      mobilePortrait: 'بورتريه المحمول',
    },
  },
  panels: {
    buttons: {
      titles: {
        preview: 'معاينة',
        fullscreen: 'كامل الشاشة',
        'sw-visibility': 'عرض المكونات',
        'export-template': 'عرض الكود',
        'open-sm': 'افتح مدير النمط',
        'open-tm': 'إعدادات',
        'open-layers': 'افتح مدير الطبقة',
        'open-blocks': 'كتل مفتوحة',
      },
    },
  },
  selectorManager: {
    label: 'الطبقات',
    selected: 'المحدد',
    emptyState: '- ولاية -',
    states: {
      hover: 'يحوم',
      active: 'انقر',
      'nth-of-type(2n)': 'Even/Odd',
    },
  },
  styleManager: {
    empty: 'حدد عنصرًا من قبل',
    layer: 'طبقة',
    fileButton: 'الصور',
    sectors: {
      general: 'عام',
      layout: 'تَخطِيط',
      typography: 'الطباعة',
      decorations: 'زينة',
      extra: 'إضافي',
      flex: 'ثني',
      dimension: 'البعد',
    },
    // Default names for sub properties in Composite and Stack types.
    // Other labels are generated directly from their property names (eg. 'font-size' will be 'Font size').
    properties: {
      'text-shadow-h': 'X',
      'text-shadow-v': 'Y',
      'text-shadow-blur': 'طمس',
      'text-shadow-color': 'اللون',
      'box-shadow-h': 'X',
      'box-shadow-v': 'Y',
      'box-shadow-blur': 'طمس',
      'box-shadow-spread': 'الانتشار',
      'box-shadow-color': 'اللون',
      'box-shadow-type': 'يكتب',
      'margin-top-sub': 'قمة',
      'margin-right-sub': 'حق',
      'margin-bottom-sub': 'الأسفل',
      'margin-left-sub': 'اليسار',
      'padding-top-sub': 'قمة',
      'padding-right-sub': 'حق',
      'padding-bottom-sub': 'الأسفل',
      'padding-left-sub': 'اليسار',
      'border-width-sub': 'عرض',
      'border-style-sub': 'أسلوب',
      'border-color-sub': 'اللون',
      'border-top-left-radius-sub': 'أعلى اليسار',
      'border-top-right-radius-sub': 'فوق على اليمين',
      'border-bottom-right-radius-sub': 'أسفل اليمين',
      'border-bottom-left-radius-sub': 'أسفل اليسار',
      'transform-rotate-x': 'تدوير X',
      'transform-rotate-y': 'استدارة Y',
      'transform-rotate-z': 'تدوير Z',
      'transform-scale-x': 'مقياس X',
      'transform-scale-y': 'مقياس Y',
      'transform-scale-z': 'مقياس Z',
      'transition-property-sub': 'خاصية',
      'transition-duration-sub': 'مدة',
      'transition-timing-function-sub': 'توقيت',
      'background-image-sub': 'صورة',
      'background-repeat-sub': 'يكرر',
      'background-position-sub': 'موضع',
      'background-attachment-sub': 'مرفق',
      'background-size-sub': 'بحجم',
      'background-color':'لون الخلفية',
      'background':'معرفتي',
      'display':'عرض',
      'float':'تطفو',
      'position':'وضع',
      "top":'أعلى',
      "right":'حق',
      "left":'اليسار',
      'bottom':'الأسفل',
      "width":'العرض',
      "height":'ارتفاع',
      "min-height":'أدنى ارتفاع',
      "max-width":'أقصى عرض',
      "margin":'حافة',
      "padding":'حشوة',
      "font-family":'خط العائلة',
      "font-size":'حجم الخط',
      "font-weight":'وزن الخط',
      "letter-spacing":'تباعد الحروف',
      "color":'اللون',
      "line-height":'ارتفاع خط',
      "text-align":'محاذاة النص',
      "text-shadow":'ظل النص',
      "border-radius":'نصف قطر الحد',
      "border":'الحدود',
      "box-shadow":'مربع الظل',
      "opacity":'العتامة',
      "transition":'انتقال',
      "transform":'تحول',

    },
    // Translate options in style properties
    // options: {
    //   float: { // Id of the property
    //     ...
    //     left: 'Left', // {option id}: {Option label}
    //   }
    // }
  },
  traitManager: {
    empty: 'حدد عنصرًا قبل استخدام مدير السمات',
    label: 'إعدادات المكون',
    traits: {
      // The core library generates the name by their `name` property
      labels: {
        // id: 'Id',
        // alt: 'Alt',
        // title: 'Title',
        // href: 'Href',
      },
      // In a simple trait, like text input, these are used on input attributes
      attributes: {
        id: traitInputAttr,
        alt: traitInputAttr,
        title: traitInputAttr,
        href: { placeholder: 'eg. https://google.com' },
      },
      // In a trait like select, these are used to translate option names
      options: {
        target: {
          false: 'This window',
          _blank: 'New window',
        },
      },
    },
  },
  storageManager: {
    recover: 'Do you want to recover unsaved changes?',
  },
};



export const english = {
  assetManager: {
    addButton: 'Add image',
    inputPlh: 'http://path/to/the/image.jpg',
    modalTitle: 'Select Image',
    uploadTitle: 'Drop files here or click to upload',
  },
  // Here just as a reference, GrapesJS core doesn't contain any block,
  // so this should be omitted from other local files
  blockManager: {
    labels: {
      // 'block-id': 'Block Label',
      'link-block': 'Link-Block',
      'quote': 'Quotes',
      'text-basic': 'Text Section',
      'column1': '1 Column',
      'column2': '2 Column',
      'column3': '3 Column',
      'column3-7': '4 Column',
      'text': 'Text',
      'link': 'Link',
      'image': 'Photo',
      'video': 'Video',
      'map': 'Map'
    },
    categories: {
      // 'Image': 'Photo'

      // 'category-id': 'Category Label',
    },
  },
  domComponents: {
    names: {
      '': 'Box',
      wrapper: 'Body',
      text: 'Text',
      comment: 'Comment',
      Image: 'pic',
      video: 'Video',
      label: 'Label',
      link: 'Link',
      map: 'Map',
      tfoot: 'Table foot',
      tbody: 'Table body',
      thead: 'Table head',
      table: 'Table',
      row: 'Table row',
      cell: 'Table cell',
    },
  },
  deviceManager: {
    device: 'Device',
    devices: {
      desktop: 'Desktop',
      tablet: 'Tablet',
      mobileLandscape: 'Mobile Landscape',
      mobilePortrait: 'Mobile Portrait',
    },
  },
  panels: {
    buttons: {
      titles: {
        preview: 'Preview',
        fullscreen: 'Fullscreen',
        'sw-visibility': 'View components',
        'export-template': 'View code',
        'open-sm': 'Open Style Manager',
        'open-tm': 'Settings',
        'open-layers': 'Open Layer Manager',
        'open-blocks': 'Open Blocks',
      },
    },
  },
  selectorManager: {
    label: 'Classes',
    selected: 'Selected',
    emptyState: '- State -',
    states: {
      hover: 'Hover',
      active: 'Click',
      'nth-of-type(2n)': 'Even/Odd',
    },
  },
  styleManager: {
    empty: 'Select an element before',
    layer: 'Layer',
    fileButton: 'Images',
    sectors: {
      general: 'General',
      layout: 'Layout',
      typography: 'Typography',
      decorations: 'Decorations',
      extra: 'Extra',
      flex: 'Flex',
      dimension: 'Dimension',
    },
    // Default names for sub properties in Composite and Stack types.
    // Other labels are generated directly from their property names (eg. 'font-size' will be 'Font size').
    properties: {
      'text-shadow-h': 'X',
      'text-shadow-v': 'Y',
      'text-shadow-blur': 'Blur',
      'text-shadow-color': 'Color',
      'box-shadow-h': 'X',
      'box-shadow-v': 'Y',
      'box-shadow-blur': 'Blur',
      'box-shadow-spread': 'Spread',
      'box-shadow-color': 'Color',
      'box-shadow-type': 'Type',
      'margin-top-sub': 'Top',
      'margin-right-sub': 'Right',
      'margin-bottom-sub': 'Bottom',
      'margin-left-sub': 'Left',
      'padding-top-sub': 'Top',
      'padding-right-sub': 'Right',
      'padding-bottom-sub': 'Bottom',
      'padding-left-sub': 'Left',
      'border-width-sub': 'Width',
      'border-style-sub': 'Style',
      'border-color-sub': 'Color',
      'border-top-left-radius-sub': 'Top Left',
      'border-top-right-radius-sub': 'Top Right',
      'border-bottom-right-radius-sub': 'Bottom Right',
      'border-bottom-left-radius-sub': 'Bottom Left',
      'transform-rotate-x': 'Rotate X',
      'transform-rotate-y': 'Rotate Y',
      'transform-rotate-z': 'Rotate Z',
      'transform-scale-x': 'Scale X',
      'transform-scale-y': 'Scale Y',
      'transform-scale-z': 'Scale Z',
      'transition-property-sub': 'Property',
      'transition-duration-sub': 'Duration',
      'transition-timing-function-sub': 'Timing',
      'background-image-sub': 'Image',
      'background-repeat-sub': 'Repeat',
      'background-position-sub': 'Position',
      'background-attachment-sub': 'Attachment',
      'background-size-sub': 'Size',
    },
    // Translate options in style properties
    // options: {
    //   float: { // Id of the property
    //     ...
    //     left: 'Left', // {option id}: {Option label}
    //   }
    // }
  },
  traitManager: {
    empty: 'Select an element before using Trait Manager',
    label: 'Component settings',
    traits: {
      // The core library generates the name by their `name` property
      labels: {
        // id: 'Id',
        // alt: 'Alt',
        // title: 'Title',
        // href: 'Href',
      },
      // In a simple trait, like text input, these are used on input attributes
      attributes: {
        id: traitInputAttr,
        alt: traitInputAttr,
        title: traitInputAttr,
        href: { placeholder: 'eg. https://google.com' },
      },
      // In a trait like select, these are used to translate option names
      options: {
        target: {
          false: 'This window',
          _blank: 'New window',
        },
      },
    },
  },
  storageManager: {
    recover: 'Do you want to recover unsaved changes?',
  },
};


